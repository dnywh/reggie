...
async function lookup(table, filter, select = "*") {
  const url = `${SUPABASE_URL}/rest/v1/${table}?${filter}&select=${encodeURIComponent(select)}`;
...
async function create(table, payload) {
  ...
  if (!res.ok) throw new Error(`${table} create failed: ${res.status} ${await res.text()}`);
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const users = await lookup("users", `email=eq.${encodeURIComponent(from)}`, "id");
    let userId;
    if (Array.isArray(users) && users.length && users[0]?.id) {
      ...
      if (!createdUser || !createdUser.id) throw new Error("User creation did not return id");
      userId = createdUser.id;
    }
    // 2) Find or create conversation for that user
    const convs = await lookup("conversations", `user_id=eq.${encodeURIComponent(userId)}`, "id");
    let convoId;
    if (Array.isArray(convs) && convs.length && convs[0]?.id) {
      ...
      if (!createdConv || createdConv.id == null) throw new Error("Conversation creation did not return id");
