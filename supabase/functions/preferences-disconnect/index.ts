...
Deno.serve(async (req: Request) => {
    try {
        ...
        const confirmationUrl =
            `${SUPABASE_URL}/functions/v1/preferences-confirm-disconnect?token=${token}`;

        const emailContent = [
            `G'day ${displayName},`,
            "",
            "You've requested to disconnect Reggie from your Strava account and delete all your data.",
            "",
            "⚠️ WARNING: This action cannot be undone. All your training data and preferences will be permanently deleted.",
            "",
        ...
        await resend.emails.send({
            from: FROM_EMAIL,
            to: email,
            subject: "Confirm: Disconnect Reggie account",
            text: emailContent,
        });

        console.log(`✅ Disconnect confirmation email sent to ${email}`);
